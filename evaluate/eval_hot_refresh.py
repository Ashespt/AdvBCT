...
def evaluate_func(model, query_loader, gallery_loader, query_gts,logger,
                  config, old_model=None, dataset_name='gldv2',hot_refresh=False):
    model.eval()
    ...
    if dist.get_rank() == 0:
        ...
        extract_features(model,old_model, 0, query_loader, 'q', logger, config)
        ...
        def get_gallery_feature(ratio):
            extract_features(model, old_model, ratio, gallery_loader, 'g', logger,
                             config)  # use old_model to extract
            ...
            print(f'mAP: {mAP}')
        if hot_refresh:
            for refresh_ratio in [0,0.25,0.5,0.75,1]:
                logger.info(f"=> the refresh ratio of gallery {refresh_ratio}")
                get_gallery_feature(refresh_ratio)
        else:
            get_gallery_feature(0)
    return mAP


@torch.no_grad()
def extract_features(model,old_model,refresh_ratio, data_loader, tag, logger, config):
    ...
    if refresh_ratio != 0:
        random.seed(100)
        refresh_list = random.sample(range(0,data_len),int(refresh_ratio*data_len))
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.USE_AMP):
            if (refresh_ratio != 0 and i in refresh_list) or tag == 'q':
                feat = model(images.cuda())
            else:
                feat = old_model(images.cuda())
